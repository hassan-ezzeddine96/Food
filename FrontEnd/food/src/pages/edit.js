...
export default function EditFood() {
  const token = localStorage.getItem('token');
    ...
    const getData = async () => {
        axios.get(`/api/getfood/${id}`).then(res => {
            setType(res.data['type']);
            setName(res.data['name']);
            setNb(res.data['number']);
            setDir(res.data['directions']);
            let x = res.data['ingredients'].split(' ');
            x.forEach(element => {
                setArr(s => {
                    return [
                      ...s,
    ...
    const onSend=()=>{
     ...
     const Data = {Id: id,name: name, type: type, number: nb, ingredients: s, file: image, directions: dir }
     axios.put(`/api/food/${id}`,Data).then(response => {
